...
import CleaningTasksList from '../components/CleaningTasksList'
import { iCleaningTask, iCompletedCleaningTask } from '../types/cleaningTasks'
...
const CleaningMain = ({
  ...
  data: { cleaningTasks: iCleaningTask[] } | undefined
  setShowDialog: (show: boolean) => void
  setSelectedCleaningTask: (cleaningTask: iCleaningTask | null) => void
  removeCleaningTask: (id: string) => void
}) => (
  <main className='w-full pb-8 md:w-2/3 md:pl-10 xl:pl-36'>
    {completedCleaningTasks && (
      <CleaningTasksList completedCleaningTasks={completedCleaningTasks} />
    )}
    {!data && (
      <Emptylist
    ...
    {data && (
      <div className='my-12 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4'>
        {data.cleaningTasks.map((cleaningTask: iCleaningTask) => (
          <div
            key={cleaningTask._id}
          ...
          >
            <div
              className='brightness-40 h-full w-full bg-cover bg-center group-focus-within/Card:brightness-100 group-hover/Card:brightness-100 group-focus/Card:brightness-100'
              style={{
            ...
            <div className='hover-group-within/Card:opacity-100 absolute bottom-1 left-0 flex w-full justify-center gap-2 opacity-0 transition-all duration-100 ease-out group-focus-within/Card:opacity-100 group-hover/Card:opacity-100 group-focus/Card:opacity-100'>
              <Button
                ...
                onMouseClick={() => {
                  setShowDialog(true)
                  setSelectedCleaningTask(cleaningTask)
                }}
                type='submit'
                xsmall
                isFit
              ...
              <Button
                text='Borrar'
                onMouseClick={() => {
                  removeCleaningTask(cleaningTask._id)
                }}
                xsmall
                outline
                isFit
